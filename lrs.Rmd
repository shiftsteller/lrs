...
chat_id <- "Leveraged Rotation Strategy"
textChange <- "The S&P LRS Status has changed! Check https://formrtestlrs.formr.org/."
textNoChange <- "No change."

if (signals$change) {
  RCurl::url.exists(paste0("https://api.telegram.org/bot", apiToken, "/sendMessage?", "chat_id=", chat_id, "&text=", textChange))
} else {
  RCurl::url.exists(paste0("https://api.telegram.org/bot", apiToken, "/sendMessage?", "chat_id=", chat_id, "&text=", textNoChange))
}
